...
def generate_blog_content(brain_data, image_list):
    ...
    prompt = f"""
    당신은 'Crypto Oikonomos' 블로그의 전문 에디터입니다.
    오늘 날짜는 {today_str} 입니다.
    
    아래에는 두 가지 데이터가 제공됩니다.
    1. [Macro Context]: 최근 연준, 세계은행 등의 무게감 있는 거시 경제 컨텍스트입니다.
    2. [User Manual News]: 선생님(사용자)이 바탕화면에서 직접 엄선하여 복사해준 오늘의 핵심 뉴스 텍스트들입니다. 주인공은 이 뉴스들입니다.
    
    [작성 원칙 및 톤앤매너 - 매우 중요!]
    1. **Tone:** 너무 권위적이거나 딱딱한 문체를 피하고, **가독성 높고 이해하기 편한 친밀한 존댓말**로 서술하세요. (예: "~했습니다.", "~보입니다.", "흥미로운 점은 ~라는 것이죠.")
    2. **No Emojis:** 이모티콘(🚀, 💡, 📊 등)은 일절 사용하지 마세요. 깔끔하고 정돈된 텍스트만 사용합니다.
    3. **No Greetings (인사말 및 서론 절대 금지):** 문서를 시작할 때 "안녕하세요", "어느덧 몇 월 며칠이네요", "오늘의 브리핑입니다" 등과 같은 일체의 서론이나 인사말을 절대 작성하지 마세요. 글의 맨 위에는 어떠한 문장도 있어서는 안 됩니다.
    4. **Structure (Markdown):**
       - **오늘의 핵심 요약 (3줄)**: 마크다운 문서의 가장 첫 줄은 무조건 `### 오늘의 핵심 요약 (3줄)` 이라는 제목으로 시작해야 합니다. 그 위에는 어떤 텍스트도 적지 마세요.
       - **1. 시장의 맥 (Macro Insights)**: [Macro Context]와 결합해 거시 흐름을 쉽게 설명.
       - **2. 크립토 다이내믹스 (Crypto Moves)**: 선생님이 주신 [User Manual News]를 심도있으면서도 편안하게 해설.
       - **3. 에디터의 시선 (Conclusion)**: 부드럽지만 통찰력 있는 마무리 제언.

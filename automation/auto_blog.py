...
# 3. 글쓰기 프롬프트 조립
# ==========================================
def _build_writing_prompt(brain_data: dict, image_list: list, today_str: str) -> str:
...
[수치·데이터 활용 규칙]
- 수동 뉴스 또는 보강 데이터에 수치(금액, 개수, %, 날짜, 법안·기관명)가 있다면, 분석을 뒷받침하는 근거로 자연스럽게 본문에 녹이세요.
...
- 수치를 단순 나열하지 말고, "이것이 왜 중요한가"를 한 문장으로 연결해 의미를 부여하세요.

[피상적 서술 금지]
- "더욱 강화될 것으로 보입니다", "큰 변화가 예상됩니다" 같은 실체 없는 전망성 문장을 금지합니다.
...
## Hook
핵심 결론이나 반직관적 시각을 먼저 1~2문장으로 던져 독자의 관심을 끄세요.
...
# 4. Gemini 글쓰기
# ==========================================
def generate_blog_content(brain_data: dict, image_list: list) -> str:
    """Gemini로 블로그 본문 생성."""
    print("✍️ [AI Editor V4] 블로그 본문 작성 중...")
    today_str = datetime.now().strftime("%Y년 %m월 %d일")
    prompt = _build_writing_prompt(brain_data, image_list, today_str)
...
def run_automation():
    ...
    # 3. 글쓰기
    print("\n✍️ [Step 3] Gemini 글쓰기...")
    blog_body = generate_blog_content(brain_data, user_images)
